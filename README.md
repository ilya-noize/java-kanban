...
* getEndTime() - время завершения задачи, которое рассчитывается исходя из startTime и duration.
  ...
  при создании или обновлении задач все его методы будут принимать и возвращать объект, в который вы добавите два новых
  поля.
...
<p><u>ключ</u> : tasks,  subtasks,  epics,  history</p>

Используется в классе HTTPTaskManager, переопределённые методы save и load.

#### KVClient

<p>Принимает URL к серверу хранилища и регистрируется.</p>
При регистрации выдаётся токен (API_TOKEN),
который нужен при работе с сервером.

      Метод void put(String key, String json) должен сохранять
      состояние менеджера задач через запрос POST

```/save/key?API_TOKEN=.```

      Метод String load(String key) должен возвращать
      состояние менеджера задач через запрос GET
...
<p>Путь:
  <ul>
      <li> для обычных задач — /tasks/task </li>
      <li> для подзадач — /tasks/subtask </li>
      <li> для эпиков — /tasks/epic </li>
  </ul>
    <p>Методы запросов для обработки данных:
      <ul>
          <li> Получение — GET.</li>
          <li> Создание/Изменение — POST.</li>
          <li> Удаление — DELETE.</li>
      </ul>
    </p>
  <ul>
      <li> Получить все задачи: /tasks/</li>
      <li> Получить историю задач: /tasks/history</li>
  </ul>
</p>
```